...
void light_init(void) {
    ...
    // Disable digital input on PK7 (ADC15) (page 287)
...
uint16_t light_read(void) {
    uint32_t timeout = 5;//if 2cc for incrementing and evaluation the timeout is 5ms
    // Set reference voltage to AVCC and left adjust ADC result
    // The  MUX1:5 should be set to 100111 for choosing ADC15, which ius placed on PK7 (look at page 283)
    ...
    // Wait for the conversion to complete the timeout is 5ms
    while ((ADCSRA & (1 << ADSC))&& timeout > 0){timeout--; k_msleep(1);};

    // Read the 10-bit ADC value
    // ADCL must be read first, then ADCH
    uint16_t adc_value = ADCL;
    adc_value |= (ADCH << 8);
    adc_value = 1023-adc_value; // light intensity inverted.
    return adc_value;
}
